...
function WishlistCard(props) {
  ...
  const { isError, response } = useSelector(store => store.removeWishListReducer)
  ...
  useEffect(() => {
    ...
    if (response) {
      if (response === "reSuc") {
        toast.success("Product Remove From Wish List Success", { autoClose: 2000 })
        dispatch({ type: REMOVE_WISHLIST_REQUEST_SUCESS, payload: null })
      }
    }
  }, [isError, response, dispatch])
  ...
  return (
    <>
      <div className={styles.__child_container} key={props.productId._id}>
        <div className={styles.__image__} onClick={() => navigate(`/product/single/${props.productId._id}`)} >
