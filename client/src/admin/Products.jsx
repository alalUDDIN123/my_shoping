...
const Products = () => {
  ...
  const { products, isLoading , totalProductspage} = useSelector((store) => store.getProductReducer);
...
console.log(" totalProductspage:", totalProductspage);
        ...
        <table className={styles.__admin__product__table} >
          ...
          <tbody className={styles.__admin__product__body}>
            {products.length > 0 && products?.map((el, ind) => (
              <ProductTableItem key={ind} {...el} ind={ind} />
            ))}
          </tbody>
        </table>
      )}
